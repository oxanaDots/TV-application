  ...
  useEffect(() => {
    async function helper() {
      ...
      if (directory.length !== 0 && detailsDir.exists) {
        ...
        const expireBoolean =
          currentExhibitionExpireDate === todaysDate.slice(0, 10);
  ...
  useEffect(() => {
    async function helper() {
      try {
        ...
        const detailsDir = await FileSystem.getInfoAsync(
    ...
    <View testID="display-cont" style={styles.displayCont}>
      {!hasImages && (
        <View style={styles.cont}>
          <Text
            style={{ fontWeight: "800", fontSize: 20, paddingVertical: 10 }}
          >
            No exhibitions to show{" "}
          </Text>
            ...
            <View style={{ display: "flex" }}>
              <View style={styles.textNameCont}>
                <Text style={{ fontWeight: "800" }}>
                  {details.artistFirstName} {details.artistLastName}
                </Text>
              </View>
