...
describe("", () => {
  ...
  it("Reads content length of the main directory", async () => {
    const images = Array.from(
      { length: 5 },
      (_, i) => `${documentDirectory}/exhibition/images/image${i}.jpg`,
    );
    const returnedDetails = { exists: true };
    const detailsObjs = {
      artistFirstName: "Firstname",
      artistsLAstNAme: "Lastname",
      title: "Title Test",
    };
  ...
  it("", async () => {
    ...
    expect(
      readDirectoryAsync("file:///exhibitions/images/image1.jpg"),
    ).rejects.toThrow("Incorrect format");
  });
});
